...
@check_memory("TapHold")
def create(input: tuple[str, str], key_names: list[str], delay: float = 0.3) -> None:
    print("Creating TapHold")
    layer, switch = input
    key_names = [str.upper(name) for name in key_names]

    # Create a standard key, and get the key StateMachine
    key = get_key((layer, switch), key_names[0]).key
